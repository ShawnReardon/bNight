namespace SpriteKind {
    export const Attack1 = SpriteKind.create()
}
scene.onOverlapTile(SpriteKind.Player, assets.tile`myTile0`, function (sprite, location) {
    info.changeScoreBy(1)
    tiles.setTileAt(location, sprites.dungeon.darkGroundNorthWest1)
    Blood_Warrior.say("I found riches.", 3000)
})
controller.up.onEvent(ControllerButtonEvent.Pressed, function () {
    Blood_Warrior.setImage(assets.image`left1`)
})
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    Blood_Warrior.setKind(SpriteKind.Attack1)
    if (controller.dx() > 0) {
        Blood_Warrior.setImage(img`
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . 2 2 b . . . . . . . . . . 
            . . b 2 b b b . . . . . . . . . 
            . b b c c c b b . . e . . . . . 
            . b b b c b 2 b b b e d 2 d d . 
            . b b b c b b 2 . . e . . . . . 
            . . b c b 2 b . . . . . . . . . 
            . . . b b b . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            `)
        animation.runImageAnimation(
        Blood_Warrior,
        assets.animation`AttackRight`,
        100,
        false
        )
        Blood_Warrior.startEffect(effects.spray, 500)
    } else if (controller.dx() < 0) {
        Blood_Warrior.setImage(assets.image`leftAttack`)
        animation.runImageAnimation(
        Blood_Warrior,
        assets.animation`AttackLeft`,
        100,
        false
        )
        Blood_Warrior.setVelocity(-100, 0)
        Blood_Warrior.startEffect(effects.coolRadial, 500)
    }
    pause(500)
    Blood_Warrior.setKind(SpriteKind.Player)
})
controller.left.onEvent(ControllerButtonEvent.Pressed, function () {
    Blood_Warrior.setImage(assets.image`leftAttack`)
})
sprites.onOverlap(SpriteKind.Attack1, SpriteKind.Enemy, function (sprite, otherSprite) {
    Rotting = sprites.create(img`
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . 7 7 7 . . . 7 7 7 . . . . . 
        . . 7 2 7 . . . 7 2 7 . . . . . 
        . . 7 7 7 . . . 7 7 7 . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . 7 7 7 . . . . . . . 
        . . . . . . 7 2 7 . . . . . . . 
        . . . . . . 7 7 7 . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . 7 7 7 . . . . 
        . . 7 7 7 . . . . 7 2 7 . . . . 
        . . 7 2 7 . . . . 7 7 7 . . . . 
        . . 7 7 7 . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        `, SpriteKind.Food)
    tiles.placeOnTile(Rotting, tiles.locationOfSprite(otherSprite))
    otherSprite.destroy(effects.fire, 500)
})
controller.right.onEvent(ControllerButtonEvent.Pressed, function () {
    Blood_Warrior.setImage(assets.image`Undead Warrior`)
})
controller.down.onEvent(ControllerButtonEvent.Pressed, function () {
    Blood_Warrior.setImage(assets.image`left`)
})
info.onLifeZero(function () {
    setup(true)
})
function setup (isRespawn: boolean) {
    for (let value of sprites.allOfKind(SpriteKind.Enemy)) {
        value.destroy()
    }
    if (isRespawn) {
        info.setLife(3)
    }
    spawnCounter = 0
    tiles.placeOnTile(Blood_Warrior, tiles.getTileLocation(col[lvl], row[lvl]))
}
sprites.onOverlap(SpriteKind.Player, SpriteKind.Food, function (sprite, otherSprite) {
    otherSprite.destroy()
    info.changeLifeBy(1)
})
scene.onOverlapTile(SpriteKind.Player, sprites.dungeon.stairLarge, function (sprite, location) {
    tiles.setTileAt(location, sprites.dungeon.darkGroundNorthWest1)
    lvl += 1
    setup(false)
})
sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, function (sprite, otherSprite) {
    otherSprite.destroy()
    scene.cameraShake(4, 500)
    info.changeLifeBy(-1)
})
let Disembodied_Soul: Sprite = null
let spawnCounter = 0
let Rotting: Sprite = null
let row: number[] = []
let col: number[] = []
let lvl = 0
let Blood_Warrior: Sprite = null
Blood_Warrior = sprites.create(assets.image`Undead Warrior`, SpriteKind.Player)
Blood_Warrior.setPosition(27, 5)
controller.moveSprite(Blood_Warrior, 100, 100)
info.setScore(0)
game.splash("Bloodnight")
scene.cameraFollowSprite(Blood_Warrior)
scene.setBackgroundImage(img`
    
    `)
tiles.setTilemap(tilemap`level1`)
lvl = 0
col = [1, 47, 24, 2, 47]
row = [0, 2, 23, 47, 47]
setup(true)
game.onUpdateInterval(1000, function () {
    if (spawnCounter < 5) {
        for (let value of tiles.getTilesByType(assets.tile`myTile1`)) {
            Disembodied_Soul = sprites.create(img`
                . . . . . . . . . . . . . . . . 
                . . . . . . . . . . . . . . . . 
                . . . . e e e e . . . . . . . . 
                . . . e e 2 e e e . . . . . . . 
                . . d f d d 2 f 2 . . . . . . . 
                . . d d d 2 d 2 d . . . . . . . 
                . . d d 2 2 2 d 2 . . . . . . . 
                . . . d d 2 d 2 2 . . . . . . . 
                . . . 2 d 2 2 d . . . . . . . . 
                . . . . 2 d d 2 . . . . . . . . 
                . . . . . . . . . . . . . . . . 
                . . . . . . . . . . . . . . . . 
                . . . . . . . . . . . . . . . . 
                . . . . . . . . . . . . . . . . 
                . . . . . . . . . . . . . . . . 
                . . . . . . . . . . . . . . . . 
                `, SpriteKind.Enemy)
            tiles.placeOnTile(Disembodied_Soul, value)
        }
        spawnCounter += 1
    }
})
forever(function () {
    for (let value2 of sprites.allOfKind(SpriteKind.Enemy)) {
        if (tiles.locationXY(tiles.locationOfSprite(Blood_Warrior), tiles.XY.column) - tiles.locationXY(tiles.locationOfSprite(value2), tiles.XY.column) == 5 || tiles.locationXY(tiles.locationOfSprite(Blood_Warrior), tiles.XY.row) - tiles.locationXY(tiles.locationOfSprite(value2), tiles.XY.row) == 5) {
            value2.follow(Blood_Warrior, randint(20, 50))
        } else {
        	
        }
    }
})
